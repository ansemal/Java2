...
public class Solution {
        ...
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                if (result ==0) {
                    ...
                    Long date1 = ((Date) stock1.get("date")).getTime() / oneDay;
                    Long date2 = ((Date) stock2.get("date")).getTime() / oneDay;
                    result = date2.compareTo(date1);
                    if (result ==0)
                        result = getProfit(stock2) - getProfit(stock1);
                }
                return result;
            }
        });
    ...
    public static int getProfit(Stock stock) {
        double result;
        if (stock.containsKey("change"))
            result = !stock.containsKey("change") ? 0 : ((double) stock.get("change"));
